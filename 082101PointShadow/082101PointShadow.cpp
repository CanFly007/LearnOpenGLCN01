...
int main()
{
	...
	glUseProgram(modelShader.program);
	glUniform1i(glGetUniformLocation(modelShader.program, "woodTexture"), 0);
	glUniform1i(glGetUniformLocation(modelShader.program, "depthCubemap"), 1);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//ShadowMapping
		//设置物体到光源空间的View和Projection矩阵  右、左、上、下、近、远
		...
		for (GLuint i = 0; i < 6; i++)
		{
			glUniformMatrix4fv(glGetUniformLocation(shadowMapShader.program, ("lightSpacePVs[" + std::to_string(i) + "]").c_str()), 1, GL_FALSE, glm::value_ptr(shadowTransforms[i]));
		}
		glUniform1f(glGetUniformLocation(shadowMapShader.program, "far_plane"), far_plane);
		glUniform3fv(glGetUniformLocation(shadowMapShader.program, "worldLightPos"), 1, &lightPos[0]);
		...
		glUniform3fv(glGetUniformLocation(modelShader.program, "worldLightPos"), 1, &lightPos[0]);
		glUniform3fv(glGetUniformLocation(modelShader.program, "worldCameraPos"), 1, &camera.Position[0]);
		glUniform1f(glGetUniformLocation(modelShader.program, "far_plane"), far_plane);
