...
int main()
{
	...
	Shader normalVisualShader("Point.vs", "Point.fs", "Point.gs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		SetViewProjectionMat4(cubeShader);
		...
		//使用几何着色器显示模型法线
		glUseProgram(normalVisualShader.program);
		SetViewProjectionMat4(normalVisualShader);
		model = glm::mat4(1.0f);
		glUniformMatrix4fv(glGetUniformLocation(normalVisualShader.program, "model"), 1, GL_FALSE, glm::value_ptr(model));
		glBindVertexArray(cubeVAO);
		glDrawArrays(GL_TRIANGLES, 0, 36);
		glBindVertexArray(0);
